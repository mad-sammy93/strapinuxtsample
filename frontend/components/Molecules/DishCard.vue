...
<template>
  <div class="bg-white rounded-lg overflow-hidden shadow-lg flex">
    ...
    <div class="p-10">
      <NuxtLink :to="`/dish/${id}`"><h4 class="text-green-700 text-2xl font-semibold mb-2">{{ dishInfo?.dish?.data?.attributes?.Name }}</h4></NuxtLink>
      <p class="text-gray-500 text-lg mb-2">{{ dishInfo?.dish?.data?.attributes?.description }}</p>
    </div>
  </div>
</template>